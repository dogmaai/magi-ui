...
function majorityVote(answers) {
  const map = new Map();
  for (const a of answers) { const k = _normForVote(a); map.set(k, (map.get(k) || 0) + 1); }
  let winner = null, count = 0;
  for (const [k, c] of map.entries()) { if (c > count) { count = c; winner = k; } }
...
async function judgeByGPT(prompt, candidates) {
  ...
  try { const js = JSON.parse(m[0]); return { index: Number(js.winner) || 0, reason: String(js.reason || 'judge') }; }
  catch { return { index: 0, reason: 'parse_fallback' }; }
}
...
app.post('/consensus', async (req, res) => {
  try {
    ...
    if (targetProviders.includes('openai')) {
      const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
      if (!OPENAI_API_KEY) { tasks.push(Promise.resolve({ provider: 'openai', model: models.openai || null, output: '', latency_ms: null, error: 'OPENAI_API_KEY not set' })); }
      else {
        const mdl = models.openai || process.env.OPENAI_MODEL || 'gpt-4o-mini';
        const body = { model: mdl, messages: [ ...(system ? [{ role: 'system', content: system }] : []), { role: 'user', content: prompt } ], temperature };
        if (max_tokens) body.max_tokens = max_tokens;
        const started = Date.now();
        const p = httpPostJson('https://api.openai.com/v1/chat/completions', { headers: { Authorization: `Bearer ${OPENAI_API_KEY}` }, body, timeoutMs: timeout_ms })
          .then(r => r.ok ? { provider: 'openai', model: r.data?.model || mdl, output: normalizeText(r.data?.choices?.[0]?.message?.content ?? ''), latency_ms: r.latency ?? (Date.now()-started) }
                          : { provider: 'openai', model: mdl, output: '', latency_ms: r.latency ?? (Date.now()-started), error: r.data?.error?.message || r.text || `upstream_error_${r.status}` });
        tasks.push(p);
      }
    }
    if (targetProviders.includes('gemini')) {
      const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
      if (!GEMINI_API_KEY) { tasks.push(Promise.resolve({ provider: 'gemini', model: models.gemini || null, output: '', latency_ms: null, error: 'GEMINI_API_KEY not set' })); }
      else {
        const mdl = models.gemini || process.env.GEMINI_MODEL || 'gemini-1.5-flash';
        ...
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(mdl)}:generateContent?key=${encodeURIComponent(GEMINI_API_KEY)}`;
        const started = Date.now();
        const p = httpPostJson(url, { body, timeoutMs: timeout_ms })
          .then(r => r.ok ? { provider: 'gemini', model: mdl, output: normalizeText((r.data?.candidates?.[0]?.content?.parts || []).map(p => p.text)), latency_ms: r.latency ?? (Date.now()-started) }
                          : { provider: 'gemini', model: mdl, output: '', latency_ms: r.latency ?? (Date.now()-started), error: r.data?.error?.message || r.text || `upstream_error_${r.status}` });
        tasks.push(p);
      }
    }
    if (targetProviders.includes('xai')) {
      const XAI_API_KEY = process.env.XAI_API_KEY;
      if (!XAI_API_KEY) { tasks.push(Promise.resolve({ provider: 'xai', model: models.xai || null, output: '', latency_ms: null, error: 'XAI_API_KEY not set' })); }
      else {
        const mdl = models.xai || process.env.XAI_MODEL || 'grok-2-latest';
        const body = { model: mdl, messages: [ ...(system ? [{ role: 'system', content: system }] : []), { role: 'user', content: prompt } ], temperature, ...(max_tokens ? { max_tokens } : {}) };
        const started = Date.now();
        const p = httpPostJson('https://api.x.ai/v1/chat/completions', { headers: { Authorization: `Bearer ${XAI_API_KEY}` }, body, timeoutMs: timeout_ms })
          .then(r => r.ok ? { provider: 'xai', model: r.data?.model || mdl, output: normalizeText(r.data?.choices?.[0]?.message?.content ?? ''), latency_ms: r.latency ?? (Date.now()-started) }
                          : { provider: 'xai', model: mdl, output: '', latency_ms: r.latency ?? (Date.now()-started), error: r.data?.error?.message || r.text || `upstream_error_${r.status}` });
        tasks.push(p);
      }
    }
