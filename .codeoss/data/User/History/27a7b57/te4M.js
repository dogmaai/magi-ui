...
async function callInternalApi(path, { method = 'POST', body, headers = {} } = {}) {
  if (!targetAudience) throw new Error('API_URL is undefined');
  const auth = new GoogleAuth();
  const client = await auth.getIdTokenClient(targetAudience); // UI実行SAでIDトークン
  const url = `${targetAudience}${path}`;
  const res = await client.request({
    url, method,
    headers: { 'Content-Type': 'application/json', ...headers },
...
/* ------------------------------ /compare --------------------------------- */
app.post('/compare_direct', async (req, res) => {
  try {
...
app.post('/consensus_direct', async (req,res)=>{
  try{
    const {prompt,system,providers,models={},temperature=0.2,timeout_ms=30000,max_tokens}=req.body||{};
    if(!prompt||typeof prompt!=='string') return res.status(400).json({error:'prompt is required (string)'});
...
// === Proxy routes (UI -> magi-app) ===
app.post('/compare', async (req, res) => {
  try {
    const data = await callInternalApi('/compare', { body: req.body }); // magi-appへ委譲
    res.json(data);
  } catch (e) {
...
app.post('/consensus', async (req, res) => {
  try {
    const data = await callInternalApi('/consensus', { body: req.body }); // magi-appへ委譲
    res.json(data);
  } catch (e) {
    res.status(e?.response?.status || 500).json({ error: e?.message, detail: e?.response?.data });
  }
